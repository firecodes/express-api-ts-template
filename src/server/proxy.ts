...
import { isNodeProd } from './environment';
import { FORBIDDEN, BAD_REQUEST, INVALID_ERROR } from '../constants/http-code';
import { PROXY_PREFIX, PROXY_ALLOWLIST_REGEXP, getStaticURL } from '../config';
import { META } from '../config/app.config';

export const PROXY_ROUTE_PATH = `${PROXY_PREFIX}/*`;
...
export const proxyer = (): RequestHandler => {
  ...
  proxy.on('proxyRes', (proxyResponse) => {
    ...
    if ([301, 302, 307, 308].includes(statusCode) && location) {
      proxyResponse.headers.location = `${PROXY_PREFIX}/${btoa(location)}`;
    }
    if (statusCode === 200) {
  ...
  proxy.on('error', (error: any, request: any, response: any, target: any) => {
    ...
    console.warn(`[BLOG] proxy error: ${error.message} > ${target?.href}`);
  ...
  return (request, response) => {
    ...
    if (isNodeProd) {
      ...
      const isAllowedReferer = !referer || PROXY_ALLOWLIST_REGEXP.test(referer);
      const isAllowedOrigin = !origin || PROXY_ALLOWLIST_REGEXP.test(origin);
